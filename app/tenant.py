...
def kubeconfig():
    ...
    # Add a helm repo if
    custom_check_output('helm repo add bitnami https://charts.bitnami.com/bitnami')
...
def odoo_value_overrides(build: BuildParam, upgrade: bool=False):
    odoo_values = {
            ...
            'secrets': [
                {
                    ...
                    'certificate': os.environ['TLS_CERTIFICATE']
                }
            ]
        },
        'extraEnv': [
            {
                'name': 'INIT_SCHOOL_ADMIN_PASSWORD',
                'value': build.odoo_school_init_password
            }
        ],
        'extraVolumeMounts': [{
            'name': 'odoo-data',
            'mountPath': '/opt/bitnami/odoo/logs',
            'subPath': 'logs'
        }],
        'affinity': {
            'podAffinity': {
        ...
        'smtpHost': build.smtp_host,
        'smtpPort': build.smtp_port,
        'smtpUser': build.smtp_user,
        'smtpPassword': build.smtp_password,
        'smtpProtocol': build.smtp_protocol,
    }
